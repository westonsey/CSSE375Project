...
public class BuildingManager {
    ...
    public boolean canUpgradeSettlement(Settlement s, BuildingType upgrade) {
        Player owner = s.getOwner();
        boolean hasResources = owner.hasResources(upgrade.getRequiredResources());
        return board.canUpgradeSettlement(s) && playerTurnManager.getTurnPhase() == TurnPhase.PLAYING_TURN && hasResources;
    }
    ...
    public void upgradeSettlement(Settlement s, BuildingType upgrade) {
        if (canUpgradeSettlement(s, upgrade)) {
            actionHandler.upgradeSettlementAllowed(s, upgrade);
        } else {
